...
final class Plugin
{
    ...
    /**
     * Fuehrt eine Boot-Methode geschuetzt aus: faengt VaultException ab, wenn die
     * WPAIS_ENCRYPTION_KEY-Konstante in wp-config.php fehlt, und zeigt stattdessen
     * einen Admin-Hinweis — die Seite laedt trotz fehlendem Schluessel weiter.
     */
    private function bootSafely(callable $bootFn, string $serviceLabel): void
    {
        try {
            $bootFn();
        } catch (VaultException $e) {
            add_action('admin_notices', static function () use ($e, $serviceLabel): void {
                printf(
                    '<div class="notice notice-error"><p>%s</p></div>',
                    esc_html(sprintf(
                        /* translators: 1: service label, 2: technical error message from ApiKeyVault */
                        __('WP AI Suite: %1$s — %2$s', 'wp-ai-suite'),
                        $serviceLabel,
                        $e->getMessage()
                    ))
                );
    ...
    {
        $this->bootSafely(
            fn () => $this->container->get(ProviderSettingsPage::class)->register(),
            __('Provider-Einstellungen konnten nicht geladen werden', 'wp-ai-suite'),
        );
    }
    ...
    private function bootConversationServices(): void
    {
        ...
        }, __('Chat-Services konnten nicht gestartet werden', 'wp-ai-suite'));
    }
    ...
    private function bootKnowledgeServices(): void
    {
        ...
        }, __('Knowledge-Services konnten nicht gestartet werden', 'wp-ai-suite'));
    }
    ...
    private function registerSecurityServices(): void
    {
        ...
        $this->container->set(EncryptionKeyNotice::class, static function (): EncryptionKeyNotice {
            return new EncryptionKeyNotice();
        });
    }
    ...
    private function bootSecurityServices(): void
    {
        ...
        // M0/M1: Zeigt Setup-Hinweis wenn WPAIS_ENCRYPTION_KEY fehlt, unabhaengig davon
        // ob andere Services (Provider, Chat, Knowledge) booten konnten.
        $this->container->get(EncryptionKeyNotice::class)->register();
    }
}
